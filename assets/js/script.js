...
function setupNavigation() {
  ...
  function navigateToPage(pageName) {
    ...
    const activeLink = Array.from(navigationLinks).find(
      (nav) => nav.innerText.toLowerCase() === pageName
    );
    if (activeLink) {
    ...
    filterBtn.forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        filterItems.forEach((item) => {
          if (
            selectedValue === "all" ||
            item.dataset.category === selectedValue
          ) {
...
function setupFilterDropdown() {
  ...
  document.addEventListener("click", function (event) {
    if (
      !filterSelectBox.contains(event.target) &&
      !filterSelectButton.contains(event.target)
    ) {
      filterSelectBox.classList.remove("active");
      selectList.style.display = "none";
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/html_components/contact.html")
    ...
    .catch((error) =>
      console.error("Error loading /html_components/contact.html:", error)
    );
});
